// Program Name : TwoStructsDeclarationMethod_02.c
...
MyPoint point;	// Declaring A Single Variable Of Type 'struct MyPoint' globally ...
MyPointProperties point_properties; // Declaring A Single Variable Of Type 'struct MyPointProperties' globally....

int main(void)
{
	// Code
	// User Input For The Members Of 'struct MyPoint' Variable 'point_1'
	printf("\n======================================================================\n");
	printf("Enter X-coordinate for Point 1 : ");
	scanf("%d", &point.x_axis);
	printf("Enter Y-coordinates for Point 2 : ");
	...
	else // Atleast One Of The Two Values (either 'X' Or 'Y' Or Both) Is A non-zero Value...
	{
		...
		else if (point.y_axis == 0) // If 'Y' Is Zero.. Obviously 'X' Is The Non-Zero Value
		{
			...
			point_properties.quadrant = 0; // A Point Lying On ANy Of The Co-ordinates Axis Is Not A Part Of Any Quadrant ...
			printf("The Point Lies On The %s Axis !!!\n\n", point_properties.axis_location);
		}
